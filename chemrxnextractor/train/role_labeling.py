...
def train(model_args, data_args, train_args):
    ...
    def compute_metrics(predictions, label_ids) -> Dict:
        ...
        y_true_entities = [entity for sublist in label_list for entity in bio_entity(sublist)]
        y_pred_entities = [entity for sublist in preds_list for entity in bio_entity(sublist)]
        labels = ['O', 'material', 'temperature', 'duration', 'container', 'sample', 'solvent', 'product', 'revolution', 'times', 'yield', 'rate']
        conf_matrix = confusion_matrix(y_true_entities, y_pred_entities, labels = labels)
        #print(conf_matrix)
        # 使用seaborn库绘制混淆矩阵热图
        plt.figure(figsize=(16, 12))
        ax = sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues', vmin = 1,vmax = 300, xticklabels=labels, yticklabels=labels)
        ...
        return {
           ...
           #"entity_report": classification_report(label_list, preds_list),
