...
class BertForTagging(BertForTokenClassification):
    ...
    def forward(
        ...
        #sequence_output = self.swiglu(self.fc(sequence_output))
...
class BertCRFForRoleLabeling(BertForTokenClassification):
    def __init__(
        ...
        self.fc = nn.Linear(hidden_size, config.hidden_size)
...
#设置是否对实体跨度进行编码
        if self.prod_pool_type == "span":
            ...
            prod_h = self.pooler.pool_head(sequence_output, prod_start_mask)
        prod_h = self.ConditionIntegrator(sequence_output, prod_h)
        extended_prod_h = self.dropout(prod_h)
        ...
        if self.use_cls:
            ...
            sequence_output = torch.cat([sequence_output, extended_cls_h], 2)
        sequence_output = self.dropout(sequence_output)
        sequence_output = self.layer_norm(self.fc(sequence_output))
